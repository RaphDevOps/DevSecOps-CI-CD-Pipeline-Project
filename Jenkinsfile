...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'dev-sec-ops-cicd-pipeline-project-one', url: 'https://github.com/RaphDevOps/DevSecOps-CI-CD-Pipeline-Project.git'
            }
        }
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit greatengineer/reddit:latest "
                       sh "docker push greatengineer/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image greatengineer/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@13.201.61.51 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://15.206.210.116:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#2', //update and provide your channel name
